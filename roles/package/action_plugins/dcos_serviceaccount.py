...
def _ensure_dcos_security():
    ...
    raw_version = ''
    try:
        r = subprocess.check_output(['dcos', 'security', '--version'], env=dcos_path).decode()
    except:
...
def get_current_serviceaccount(name):
    ...
    sa_name = None
    try:
        r = subprocess.check_output(['dcos', 'security', 'org', 'service-accounts', 'show', '--json', name ], env=dcos_path)
        sa_name = name
    except:
        sa_name = None
...
def get_current_secret(secret):
    ...
    secret_name = None
    try:
        r = subprocess.check_output(['dcos', 'security', 'secrets', 'get', '--json', secret ], env=dcos_path)
        secret_name = secret
    except:
        secret_name = None
...
def add_to_group(name, group):
    ...
    if group is not None:
        ...
        display.vvv("command: " + ' '.join(cmd))
        try:
            display.vvv(subprocess.check_output(cmd, env=dcos_path))
            changed = True
        except:
            changed = False
